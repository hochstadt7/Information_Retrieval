...
def calculate_scores(queries_path, index_path):
    os.system(f'python vsm_ir.py create_index "./cfc-xml_corrected"')
    queries = list()
    ...
    for query_el in root.findall('QUERY'):
        ...
        # run query
        results = q.query(index_path, query["text"]).split("\n")
        results = [int(x.strip()) for x in results if len(x)]
...
if __name__ == '__main__':
    calculate_scores("./cfc-xml_corrected/cfquery.xml", "./vsm_inverted_index.json")
